...
private String ru_great_holiday="";
private String ru_big_holiday="";
private String post="";
private String WIDGET_PREF = "widget_pref";
private DatabaseHelper db;
private Cursor cursor;
private MyCalendar cal=MyCalendar.getInstance();
private static final String TAG = "myLogs";
private static final long wholeDayInMillis=86400000;
private int time;
private String millisKey="millis";
@Override
...
//alarm не правильный,значит устанавливаем его на время согласно наших настроек.
 else setAlarm(context,getTimeForAlarm(false));
 }
...
notification.defaults |=Notification.DEFAULT_SOUND;
        ...
        if(notificationManager.getNotificationChannel(channelId)==null) notificationManager.createNotificationChannel(new NotificationChannelCompat.Builder(channelId,NotificationManagerCompat.IMPORTANCE_DEFAULT).setName(res.getString(R.string.app_name)).build());
...
public Long getTimeForAlarm(boolean setDate) {
if(setDate)cal.setTodayDate();
cal.setHours(time);
cal.setMinutes(0);
long millis=cal.getTimeInMillis();
if(millis<System.currentTimeMillis()) return millis+86400000; else return millis;
}
...
Intent myIntentR1 = new Intent(context, MyScheduledReceiver.class);
 if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.JELLY_BEAN)myIntentR1.addFlags(Intent.FLAG_RECEIVER_FOREGROUND);
