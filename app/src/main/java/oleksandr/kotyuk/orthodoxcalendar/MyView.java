...
public class MyView extends AppCompatTextView {
    ...
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        ...
        if (getTypeface() != null && getTypeface().getStyle() == Typeface.BOLD && spans.length == 0 && !isExpandable())
            infoCompat.setHeading(true);
    }
        ...
        return new AccessibilityNodeProvider() {
            ...
            public AccessibilityNodeInfo createAccessibilityNodeInfo(int virtualViewId) {
                ...
                info.removeAction(AccessibilityNodeInfo.ACTION_COLLAPSE);*/
                    if (isExpandable())
                        info.addAction(isExpanded() ? AccessibilityNodeInfo.ACTION_COLLAPSE : AccessibilityNodeInfo.ACTION_EXPAND);
                    if (!isExpandable() && virtualViewId == NO_ID && spans.length == 1 && builder.getSpanStart(spans[0]) + builder.getSpanEnd(spans[0]) == builder.length())
...
int paddingLeft=getCompoundPaddingLeft();*/
                    ...
                    if (startLine != endLine) {
                        if (r.top > ((WindowManager) getContext().getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay().getHeight() - r.bottom)
                            endClick = layout.getLineRight(startLine);
                        else {
                            r = new Rect();
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT && spans.length > 0) {
                    ...
                    if (virtualViewId > NO_ID)
                        info.setCollectionItemInfo(AccessibilityNodeInfoCompat.CollectionItemInfoCompat.obtain(virtualViewId, 1, 0, 1, false));
                }
                return info.unwrap();
            }
            ...
            public boolean performAction(int virtualViewId, int action, Bundle arguments) {
                if (virtualViewId == NO_ID) return performAccessibilityAction(action, arguments);
                    ...
                    if (action == AccessibilityNodeInfo.ACTION_ACCESSIBILITY_FOCUS)
                        sendAccessibilityEventForVirtualView(AccessibilityEvent.TYPE_VIEW_ACCESSIBILITY_FOCUSED, virtualViewId);
                    else if (action == AccessibilityNodeInfo.ACTION_CLEAR_ACCESSIBILITY_FOCUS)
                        sendAccessibilityEventForVirtualView(AccessibilityEvent.TYPE_VIEW_ACCESSIBILITY_FOCUS_CLEARED, virtualViewId);
...
//else if(action== AccessibilityNodeInfoCompat.AccessibilityActionCompat.ACTION_SHOW_ON_SCREEN.getId()) return false;
    ...
    public boolean performAccessibilityAction(int action, Bundle arguments) {
        if (action == AccessibilityNodeInfo.ACTION_ACCESSIBILITY_FOCUS || action == AccessibilityNodeInfo.ACTION_FOCUS)
            accessibilityFocusedId = -1;
    ...
    protected boolean dispatchHoverEvent(MotionEvent event) {
        ...
        if (!touch) return super.dispatchHoverEvent(event);
        else return touch;
    }
