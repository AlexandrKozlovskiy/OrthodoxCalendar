package oleksandr.kotyuk.orthodoxcalendarfree.db;
...
public class DatabaseHelperWidget extends SQLiteOpenHelper {

	static boolean update_flag_widget=false;
	...
	private static DatabaseHelperWidget dbHelper = null;

	private static SQLiteDatabase db = null;

	private final Context db_context;

	private DatabaseHelperWidget(Context context) {
	...
	public void onUpgrade(SQLiteDatabase database, int oldVersion,
			int newVersion) {
		//Toast.makeText(db_context, "Widget", Toast.LENGTH_SHORT).show();
		//executeUpdateSQLScriptZIP(database, DB_NAME_ZIP, oldVersion);
		update_flag_widget=true;
		
		if(oldVersion>1 && oldVersion<17){
			database.execSQL("DROP TABLE prayers;");
		}
		if(oldVersion>4){
		...
		if(oldVersion>16 && oldVersion<36){
			database.execSQL("DROP TABLE prayers_ru;");
			database.execSQL("DROP TABLE prayers_cs;");
		}
		if(oldVersion>22){
	...
	public static synchronized DatabaseHelperWidget getInstance(Context context) {
		Log.d(TAG, "DatabaseHelper getInstance()");
		if (dbHelper == null) {
			dbHelper = new DatabaseHelperWidget(context);
			openConnecion();
		}
		return dbHelper;
	}
